...
func remote(logger *logging.Logger) {
	...
	query := "select ct_loc.loc_desc as loc_name, pa_patmas.pmi_name as name ,pa_adm.adm_in_pat_no,ct_hospital.hosp_desc as ct_hospital_name, pac_room.room_desc as pac_room_desc,pac_bed.bed_code as pac_bed_desc,dev_id,dev_code,dev_desc,dev_type,dev_active,dev_status,dev_video_status,sip_users_name,dev_bind_id from cf_device "
	...
	for _, re := range remoteDevs {
		if re.SipUsersName != "" {
			sipUsersNames[re.DevId] = re.SipUsersName
		}
	}

	var requestRemoteDevs []*RequestRemoteDev
	for _, re := range remoteDevs {
		if re.DevType != utils.Config.DevType {
			continue
		}
